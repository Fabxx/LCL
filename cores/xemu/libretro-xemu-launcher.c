...
static bool setup(char **Paths, size_t numPaths, char *executable)
{
   ...
   snprintf(searchPath, MAX_PATH, "%s\\xemu.exe", Paths[0]);
...
static bool downloader(char **Paths, char **downloaderDirs, char **githubUrls)
{
  ...
  "bash -c 'json_data=$(curl -s -H \"Accept: application/json\" \"%s\"); "
   ...
   #ifdef __linux__
   snprintf(command, sizeof(command), 
   "wget -O %s/xemu.AppImage %s && chmod +x %s", Paths[0], url, Paths[6]);
      ...
      if (system(command) != 0) {
         ...
         if (!urlFile || !currentVersionFile || !newVersionFile) {
            ...
            if (strcmp(currentVersion, newVersion) != 0) {
               ...
               snprintf(command, sizeof(command), 
                       "wget -O %s/xemu.zip %s", Paths[0], url);
               
               #elif defined __WIN32__
               
                snprintf(command, sizeof(command),
                       "powershell -Command \"Invoke-WebRequest -Uri '%s' -OutFile '%s\\xemu.zip'\"", url, Paths[0]);
               #endif
...
static bool extractor(char **dirs)
{
   ...
   snprintf(command, sizeof(command),
            "powershell -Command \"Expand-Archive -Path '%s\\xemu.zip' -DestinationPath '%s' -Force; Remove-Item -Path '%s\\xemu.zip' -Force\"", 
            dirs[0], dirs[0], dirs[0]);
   #elif defined __APPLE__
   snprintf(command, sizeof(command), 
           "mkdir %s/tmp_dir && "
           "unzip %s/xemu.zip -d %s/tmp_dir && " 
           "mv %s/tmp_dir/* %s && " \
           "rm -rf %s/tmp_dir && "
           "rm %s/xemu.zip && "
           "chmod +x %s", 
...
bool retro_load_game(const struct retro_game_info *info)
{
   ...
   char *dirs[] = {
         "/.config/retroarch/system/xemu",
         "/.config/retroarch/system/xemu/bios",
         "/.config/retroarch/thumbnails/Microsoft - Xbox",
         "/.config/retroarch/thumbnails/Microsoft - Xbox/Named_Boxarts",
         "/.config/retroarch/thumbnails/Microsoft - Xbox/Named_Snaps",
         "/.config/retroarch/thumbnails/Microsoft - Xbox/Named_Titles",
         "/.config/retroarch/system/xemu/xemu.AppImage" // search Path for glob.
      };
   
   // Emulator build versions and URL to download. Content is generated from powershell cmds
   char *downloaderDirs[] = {
      "/.config/retroarch/system/xemu/0.Url.txt",
      "/.config/retroarch/system/xemu/1.CurrentVersion.txt",
      "/.config/retroarch/system/xemu/2.NewVersion.txt",
   };
   ...
   char *dirs[] = {
         "/Library/Application Support/RetroArch/system/xemu",
         "/Library/Application Support/RetroArch/system/xemu/bios",
         "/Library/Application Support/RetroArch/thumbnails/Microsoft - Xbox",
         "/Library/Application Support/RetroArch/thumbnails/Microsoft - Xbox/Named_Boxarts",
         "/Library/Application Support/RetroArch/thumbnails/Microsoft - Xbox/Named_Snaps",
         "/Library/Application Support/RetroArch/thumbnails/Microsoft - Xbox/Named_Titles",
         "/Library/Application Support/RetroArch/system/xemu/xemu.app/Contents/MacOS/xemu" // search Path for glob.
      };

   // Emulator build versions and URL to download. Content is generated from powershell cmds
   char *downloaderDirs[] = {
      "/Library/Application Support/RetroArch/system/xemu/0.Url.txt",
      "/Library/Application Support/RetroArch/system/xemu/1.CurrentVersion.txt",
      "/Library/Application Support/RetroArch/system/xemu/2.NewVersion.txt",
   };
   ...
   char *dirs[] = {
         "C:\\RetroArch-Win64\\system\\xemu",
         "C:\\RetroArch-Win64\\system\\xemu\\bios",
         "C:\\RetroArch-Win64\\thumbnails\\Microsoft - Xbox",
         "C:\\RetroArch-Win64\\thumbnails\\Microsoft - Xbox\\Named_Boxarts",
         "C:\\RetroArch-Win64\\thumbnails\\Microsoft - Xbox\\Named_Snaps",
         "C:\\RetroArch-Win64\\thumbnails\\Microsoft - Xbox\\Named_Titles"
      };
   
   char *downloaderDirs[] = {
      "C:\\RetroArch-Win64\\system\\xemu\\0.Url.txt",
      "C:\\RetroArch-Win64\\system\\xemu\\1.CurrentVersion.txt",
      "C:\\RetroArch-Win64\\system\\xemu\\2.NewVersion.txt",
   };
   ...
   if (strlen(executable) > 0) {
      if (info == NULL || info->path == NULL) {
         char args[512] = {0};
         snprintf(args, sizeof(args), " -full-screen");
         strncat(executable, args, sizeof(executable)-1);
      } else {
         char args[512] = {0};
         snprintf(args, sizeof(args), " -full-screen -dvd_path \"%s\"", info->path);
         strncat(executable, args, sizeof(executable)-1);
      }

      if (system(executable) == 0) {
         log_cb(RETRO_LOG_INFO, "[LAUNCHER-INFO]: Finished running xemu.\n");
         return true;
      } else {
         log_cb(RETRO_LOG_ERROR, "[LAUNCHER-ERROR]: Failed running xemu.\n");
      }
   }
   return false;
}
