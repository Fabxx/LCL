...
bool retro_load_game(const struct retro_game_info *info)
{
      ...
      char path[512] = "";
      char PPSSPP_exec[512] = "";
      const char *home = getenv("HOME");

      if (!home) {
         return false;
      }
      snprintf(path, sizeof(path), "%s/.config/retroarch/system/PPSSPP/PPSSPP*", home);

      if (glob(path, 0, NULL, &buf) == 0) {
         for (size_t i = 0; i < buf.gl_pathc; i++) {
               if (stat(buf.gl_pathv[i], &path_stat) == 0 && !S_ISDIR(path_stat.st_mode)) {
                  snprintf(PPSSPP_exec, sizeof(PPSSPP_exec), "%s", buf.gl_pathv[i]);
                  break;
               }
         }
         globfree(&buf);
      ...
      HANDLE hFind;
      char PPSSPP_exec[MAX_PATH];
      char PPSSPP_dir[256] = "C:\\RetroArch-Win64\\system\\PPSSPP";
      char searchPath[MAX_PATH];

      snprintf(searchPath, MAX_PATH, "%s\\PPSSPP*.exe", PPSSPP_dir);
      hFind = FindFirstFile(searchPath, &findFileData);

      if (hFind == INVALID_HANDLE_VALUE) {
         printf("PPSSPP not found!\n");
         return NULL;
      }
      
      snprintf(PPSSPP_exec, MAX_PATH, "%s\\%s", PPSSPP_dir, findFileData.cFileName);
      FindClose(hFind);
   ...
   printf("libretro-PPSSPP-launcher: Failed running PPSSPP. Place it in the right path and try again\n");
   return false;
}
