...
bool retro_load_game(const struct retro_game_info *info)
{
      ...
      snprintf(path, sizeof(path), "%s/Library/Application Support/RetroArch/thumbnails/Nintendo - DS", home);
      ...
      snprintf(path, sizeof(path), "%s/Library/Application Support/RetroArch/thumbnails/Nintendo - DS/Named_Boxarts", home);
      ...
      snprintf(path, sizeof(path), "%s/Library/Application Support/RetroArch/thumbnails/Nintendo - DS/Named_Snaps", home);
      ...
      snprintf(path, sizeof(path), "%s/Library/Application Support/RetroArch/thumbnails/Nintendo - DS/Named_Titles", home);
      ...
      snprintf(path, sizeof(path), "%s/Library/Application Support/RetroArch/system/melonDS", home);
      ...
      snprintf(emuList, sizeof(emuList), "%s/Library/Application Support/RetroArch/system/melonDS/melonDS*/Contents/MacOS/melonDS", home);
    ...
    "bash -c 'json_data=$(curl -s -H \"Accept: application/json\" \"https://api.github.com/repos/melonDS-emu/melonDS/releases/latest\"); "
         ...
         char download[512] = {0};
         snprintf(download, sizeof(download), "wget -O %s/melonDS.zip %s", path, url);
         ...
         if (system(download) != 0) {
            ...
            snprintf(extraction, sizeof(extraction), 
            "unzip %s/melonDS.zip -d %s && unzip %s/macOS-universal.zip -d %s && rm %s/melonDS.zip && rm %s/macOS-universal.zip", path, path, path, path, path, path);
            ...
            if (system(extraction) != 0) {
               ...
               char chmod[512] = {0};
               snprintf(chmod, sizeof(chmod), "chmod +x %s/melonDS.app/Contents/MacOS/melonDS", path);
   ...
   printf("[LAUNCHER-INFO]: Failed running melonDS.\n");
   return false;
}
