...
bool retro_load_game(const struct retro_game_info *info)
{
      ...
      char path[512] = "";
      char melonDS_exec[512] = "";
      const char *home = getenv("HOME");

      if (!home) {
         return false;
      }
      snprintf(path, sizeof(path), "%s/.config/retroarch/system/melonDS/melonDS*", home);

      if (glob(path, 0, NULL, &buf) == 0) {
         for (size_t i = 0; i < buf.gl_pathc; i++) {
               if (stat(buf.gl_pathv[i], &path_stat) == 0 && !S_ISDIR(path_stat.st_mode)) {
                  snprintf(melonDS_exec, sizeof(melonDS_exec), "%s", buf.gl_pathv[i]);
                  break;
               }
         }
         globfree(&buf);
      ...
      HANDLE hFind;
      char melonDS_exec[MAX_PATH];
      char melonDS_dir[256] = "C:\\RetroArch-Win64\\system\\melonDS";
      char searchPath[MAX_PATH];

      snprintf(searchPath, MAX_PATH, "%s\\melonDS*.exe", melonDS_dir);
      hFind = FindFirstFile(searchPath, &findFileData);

      if (hFind == INVALID_HANDLE_VALUE) {
         printf("melonDS not found!\n");
         return NULL;
      }
      
      snprintf(melonDS_exec, MAX_PATH, "%s\\%s", melonDS_dir, findFileData.cFileName);
      FindClose(hFind);
   ...
   printf("libretro-melonDS-launcher: Failed running melonDS. Place it in the right path and try again\n");
   return false;
}
