...
bool retro_load_game(const struct retro_game_info *info)
{
      ...
      if (hFind != INVALID_HANDLE_VALUE) {
         ...
         "powershell -Command \"$response = (Invoke-WebRequest -Uri 'https://api.github.com/repos/mgba-emu/mgba/releases/latest' -Headers @{Accept='application/json'}).Content | ConvertFrom-Json; "
               "$tag = $response.tag_name;"
               "$name = $response.assets[17].name;"
         ...
         if (system(downloadCmd) != 0) {
            ...
            snprintf(extractCmd, sizeof(extractCmd),
             "powershell -Command \"Expand-Archive -Path '%s\\mGBA.zip' -DestinationPath '%s' -Force; Remove-Item -Path '%s\\mGBA.zip' -Force\"", emuPath, emuPath, emuPath);
