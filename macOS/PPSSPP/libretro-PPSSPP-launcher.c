...
bool retro_load_game(const struct retro_game_info *info)
{
      ...
      snprintf(path, sizeof(path), "%s/Library/Application Support/RetroArch/thumbnails/Sony - Playstation Portable", home);
      ...
      snprintf(path, sizeof(path), "%s/Library/Application Support/RetroArch/thumbnails/Sony - Playstation Portable/Named_Boxarts", home);
      ...
      snprintf(path, sizeof(path), "%s/Library/Application Support/RetroArch/thumbnails/Sony - Playstation Portable/Named_Snaps", home);
      ...
      snprintf(path, sizeof(path), "%s/Library/Application Support/RetroArch/thumbnails/Sony - Playstation Portable/Named_Titles", home);
      ...
      snprintf(emuList, sizeof(emuList), "%s/Library/Application Support/RetroArch/system/PPSSPP/PPSSPP*/Contents/MacOS/PPSSPPSDL", home);
    ...
    "bash -c 'json_data=$(curl -s -H \"Accept: application/json\" \"https://api.github.com/repos/hrydgard/ppsspp/releases/latest\"); "
            "tag=$(echo \"$json_data\" | jq -r \".tag_name\"); "
            "name=$(echo \"$json_data\" | jq -r \".assets[1].name\"); "
         ...
         char download[256] = {0};
         snprintf(download, sizeof(download), "wget -O %s/PPSSPP.zip %s", path, url);
         ...
         if (system(download) != 0) {
            ...
            snprintf(extraction, sizeof(extraction), "unzip %s/PPSSPP.zip -d %s && rm %s/PPSSPP.zip", path, path, path);
            ...
            if (system(extraction) != 0) {
               ...
               char chmod[512] = {0};
               snprintf(chmod, sizeof(chmod), "chmod +x %s/PPSSPP*/Contents/MacOS/PPSSPPSDL", path);
