...
static bool downloader(char **Paths, char **downloaderDirs, char **githubUrls, char *executable, size_t numPaths)
{
   ...
   "bash -c 'json_data=$(curl -s -H \"Accept: application/json\" \"%s\"); "
      ...
      "wget -O %s/PPSSPP.zip %s", Paths[0], url);
         ...
         if (system(bashCommand) != 0) {
            ...
            if (!urlFile && !currentVersionFile && ! newVersionFile) {
               ...
               if (strcmp(currentVersion, newVersion) != 0) {
                  ...
                  snprintf(downloadCmd, sizeof(downloadCmd), 
                  "wget -O %s/PPSSPP.zip", Paths[0], url);
 ...
 * NOTE: Rename PPSSPPSDL.app folder in PPSSPP.app
 */
static bool extractor(char **Paths)
{
   ...
   snprintf(extractCmd, sizeof(extractCmd), 
    "mkdir %s/tmp_dir && "
           "unzip %s/PPSSPP.zip -C %s/tmp_dir && " 
           "mv %s/tmp_dir/* %s/PPSSPP.app && "
           "rm -rf %s/tmp_dir && "
           "rm %s/PPSSPP.zip && "
           "chmod +x %s", 
...
bool retro_load_game(const struct retro_game_info *info)
{
   ...
   char *downloaderDirs[] = {
      "/Library/Application Support/RetroArch/system/PPSSPP/0.Url.txt",
      "/Library/Application Support/RetroArch/system/PPSSPP/1.CurrentVersion.txt",
      "/Library/Application Support/RetroArch/system/PPSSPP/2.NewVersion.txt",
   };

   char *githubUrls[] = {
      "https://api.github.com/repos/hrydgard/ppsspp/releases/latest",
      "https://github.com/hrydgard/PPSSPP/releases/download/"
   };
