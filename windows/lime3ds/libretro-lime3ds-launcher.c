   ...
   if (strlen(executable) == 0) {
      ...
      if (system(psCommand) != 0) {
         ...
         snprintf(downloadCmd, sizeof(downloadCmd),
          "powershell -Command \"Invoke-WebRequest -Uri '%s' -OutFile '%s\\lime3ds.zip'\"", url, dirs[0]);
...
static bool extractor(char **dirs)
{
   ...
   snprintf(extractCmd, sizeof(extractCmd),
       "powershell -Command \""
               ...
               "Expand-Archive -Path '%s\\lime3ds.zip' -DestinationPath $tempDir -Force; "
               "Move-Item -Path $tempDir\\lime3ds*\\* -Destination '%s' -Force; "
               "Remove-Item -Path $tempDir -Recurse -Force; "
               "Remove-Item -Path '%s\\lime3ds.zip' -Force\"",
...
bool retro_load_game(const struct retro_game_info *info)
{
   ...
   char *dirs[] = {
         ...
         "C:\\RetroArch-Win64\\thumbnails\\Nintendo - 3DS",
         "C:\\RetroArch-Win64\\thumbnails\\Nintendo - 3DS\\Named_Boxarts",
         "C:\\RetroArch-Win64\\thumbnails\\Nintendo - 3DS\\Named_Snaps",
         "C:\\RetroArch-Win64\\thumbnails\\Nintendo - 3DS\\Named_Titles"
      };
