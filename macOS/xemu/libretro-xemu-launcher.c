...
bool retro_load_game(const struct retro_game_info *info)
{
      ...
      snprintf(path, sizeof(path), "%s/Library/Application Support/RetroArch/thumbnails/Microsoft - Xbox", home);
      ...
      snprintf(path, sizeof(path), "%s/Library/Application Support/RetroArch/thumbnails/Microsoft - Xbox/Named_Boxarts", home);
      ...
      snprintf(path, sizeof(path), "%s/Library/Application Support/RetroArch/thumbnails/Microsoft - Xbox/Named_Snaps", home);
      ...
      snprintf(path, sizeof(path), "%s/Library/Application Support/RetroArch/thumbnails/Microsoft - Xbox/Named_Titles", home);
      ...
      snprintf(path, sizeof(path), "%s/Library/Application Support/RetroArch/system/xemu", home);
      ...
      snprintf(emuList, sizeof(emuList), "%s/Library/Application Support/RetroArch/system/xemu/xemu.App/Contents/MacOS/xemu", home);
    ...
    "bash -c 'json_data=$(curl -s -H \"Accept: application/json\" \"https://api.github.com/repos/xemu-project/xemu/releases/latest\"); "
         ...
         char download[256] = {0};
         snprintf(download, sizeof(download), "wget -O %s/xemu.zip %s", path, url);
         ...
         if (system(download) != 0) {
            ...
            snprintf(extraction, sizeof(extraction), "unzip %s/xemu.zip -d %s && rm %s/xemu.zip", path, path, path);
            ...
            if (system(extraction) != 0) {
               ...
               char chmod[512] = {0};
               snprintf(chmod, sizeof(chmod), "chmod +x %s/xemu.App/Contents/MacOS/xemu", path);
   ...
   printf("[LAUNCHER-INFO]: Failed running xemu.\n");
   return false;
}
