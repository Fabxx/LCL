...
bool retro_load_game(const struct retro_game_info *info)
{
      ...
      snprintf(path, sizeof(path), "%s/Library/Application Support/RetroArch/thumbnails/Sony - Playstation 3", home);
      ...
      snprintf(path, sizeof(path), "%s/Library/Application Support/RetroArch/thumbnails/Sony - Playstation 3/Named_Boxarts", home);
      ...
      snprintf(path, sizeof(path), "%s/Library/Application Support/RetroArch/thumbnails/Sony - Playstation 3/Named_Snaps", home);
      ...
      snprintf(path, sizeof(path), "%s/Library/Application Support/RetroArch/thumbnails/Sony - Playstation 3/Named_Titles", home);
      ...
      snprintf(emuList, sizeof(emuList), "%s/Library/Application Support/RetroArch/system/rpcs3/RPCS3.app/Contents/MacOS/rpcs3", home);
    ...
    "bash -c 'json_data=$(curl -s -H \"Accept: application/json\" \"https://api.github.com/repos/RPCS3/rpcs3-binaries-mac/releases/latest\"); "
            "tag=$(echo \"$json_data\" | jq -r \".tag_name\"); "
            "name=$(echo \"$json_data\" | jq -r \".assets[0].name\"); "
            "if [ -z \"$tag\" ] || [ -z \"$name\" ] || [ \"$tag\" = \"null\" ] || [ \"$name\" = \"null\" ]; then exit 1; fi; "
            "url=\"https://github.com/RPCS3/rpcs3-binaries-mac/releases/download/$tag/$name\"; "
         ...
         char download[256] = {0};
         snprintf(download, sizeof(download), "wget -O %s/rpcs3.7z %s", path, url);
         ...
         if (system(download) != 0) {
            ...
            snprintf(extraction, sizeof(extraction), "unzip %s/pcsx2.7z -d %s && rm %s/pcsx2.7z", path, path, path);
            ...
            if (system(extraction) != 0) {
               ...
               char chmod[512] = {0};
               snprintf(chmod, sizeof(chmod), "chmod +x %s/RPCS3.app/Contents/MacOS/rpcs3", path);
      ...
      snprintf(path, sizeof(path), "%s/Library/Application Support/RetroArch/system/rpcs3/bios", home);
