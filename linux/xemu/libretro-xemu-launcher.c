...
bool retro_load_game(const struct retro_game_info *info)
{
      ...
      snprintf(path, sizeof(path), "%s/.config/retroarch/thumbnails/Microsoft - Xbox", home);
      ...
      snprintf(path, sizeof(path), "%s/.config/retroarch/thumbnails/Microsoft - Xbox/Named_Boxarts", home);
      ...
      snprintf(path, sizeof(path), "%s/.config/retroarch/thumbnails/Microsoft - Xbox/Named_Snaps", home);
      ...
      snprintf(path, sizeof(path), "%s/.config/retroarch/thumbnails/Microsoft - Xbox/Named_Titles", home);
      ...
      snprintf(path, sizeof(path), "%s/.config/retroarch/system/xemu", home);
      ...
      snprintf(emuList, sizeof(emuList), "%s/.config/retroarch/system/xemu/xemu.AppImage", home);
    ...
    "bash -c 'json_data=$(curl -s -H \"Accept: application/json\" \"https://api.github.com/repos/xemu-project/xemu/releases/latest\"); "
            ...
            "name=$(echo \"$json_data\" | jq -r \".assets[6].name\"); "
         ...
         char download[256] = {0};
         snprintf(download, sizeof(download), "wget -O %s/xemu.AppImage %s", path, url);
         ...
         if (system(download) != 0) {
            ...
            return false;
               ...
               char chmod[512] = {0};
               snprintf(chmod, sizeof(chmod), "chmod +x %s/xemu.AppImage", path);
