...
static bool downloader(char **Paths, char **downloaderDirs, char **githubUrls)
{
  ...
  "bash -c 'json_data=$(curl -s -H \"Accept: application/json\" \"%s\"); "
   ...
   #elif defined _WIN32
      snprintf(command, sizeof(command),"powershell -Command \"Invoke-WebRequest -Uri '%s' -OutFile '%s\\lime3ds.zip'\"", url, Paths[0]);
...
static bool extractor(char **dirs)
{
   ...
   snprintf(command, sizeof(command),
"powershell -Command \""
      "Expand-Archive -Path '%s\\lime3ds.zip' -DestinationPath '%s' -Force; "
      "Remove-Item -Path '%s\\lime3ds.zip' -Force; "
      "Move-Item -Path '%s\\lime3ds*\\*' -Destination '%s' -Force; "
      "Remove-Item -Path '%s\\lime3ds-2*' -Recurse -Force"
      "\"",
