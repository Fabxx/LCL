...
bool retro_load_game(const struct retro_game_info *info)
{
      ...
      HANDLE hFind;
      char emuPath[MAX_PATH] = "C:\\RetroArch-Win64\\system\\xemu";
      char biosPath[MAX_PATH] = "C:\\RetroArch-Win64\\system\\xemu\\bios";
    ...
    "powershell -ExecutionPolicy Bypass -NoProfile -Command \"$tag = (Invoke-RestMethod 'https://api.github.com/repos/xemu-project/xemu/releases/latest').tag_name; "
...
;
      ...
      if (hFind != INVALID_HANDLE_VALUE) {
         ...
         snprintf(downloadCmd, sizeof(downloadCmd),
          "powershell -Command \"Invoke-WebRequest -Uri '%s' -OutFile '%s\\xemu.zip'\"", url, emuPath);
         ...
         if (system(downloadCmd) != 0) {
            ...
            snprintf(extractCmd, sizeof(extractCmd),
             "powershell -Command \"Expand-Archive -Path '%s\\xemu.zip' -DestinationPath '%s' -Force; Remove-Item -Path '%s\\xemu.zip' -Force\"", emuPath, emuPath, emuPath);
