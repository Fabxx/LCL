...
bool retro_load_game(const struct retro_game_info *info)
{
      ...
      snprintf(path, sizeof(path), "%s/.config/retroarch/system/xemu/xemu*", home);

      if (glob(path, 0, NULL, &buf) == 0) {
         for (size_t i = 0; i < buf.gl_pathc; i++) {
               if (stat(buf.gl_pathv[i], &path_stat) == 0 && !S_ISDIR(path_stat.st_mode)) {
                  snprintf(xemu_exec, sizeof(xemu_exec), "%s", buf.gl_pathv[i]);
                  break;
               }
         }
         globfree(&buf);
      ...
      HANDLE hFind;
      char xemu_exec[MAX_PATH];
      char xemu_dir[256] = "C:\\RetroArch-Win64\\system\\xemu";
      char searchPath[MAX_PATH];

      snprintf(searchPath, MAX_PATH, "%s\\xemu*.exe", xemu_dir);
      hFind = FindFirstFile(searchPath, &findFileData);

      if (hFind == INVALID_HANDLE_VALUE) {
         printf("xemu not found!\n");
         return NULL;
      }
      
      snprintf(xemu_exec, MAX_PATH, "%s\\%s", xemu_dir, findFileData.cFileName);
      FindClose(hFind);
   ...
   printf("libretro-xemu-launcher: Failed running xemu. Place it in the right path and try again\n");
   return false;
}
