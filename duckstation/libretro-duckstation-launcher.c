...
bool retro_load_game(const struct retro_game_info *info)
{
      ...
      char path[512] = "";
      char duckstation_exec[512] = "";
      const char *home = getenv("HOME");

      if (!home) {
         return false;
      }
      snprintf(path, sizeof(path), "%s/.config/retroarch/system/duckstation/duckstation*", home);

      if (glob(path, 0, NULL, &buf) == 0) {
         for (size_t i = 0; i < buf.gl_pathc; i++) {
               if (stat(buf.gl_pathv[i], &path_stat) == 0 && !S_ISDIR(path_stat.st_mode)) {
                  snprintf(duckstation_exec, sizeof(duckstation_exec), "%s", buf.gl_pathv[i]);
                  break;
               }
         }
         globfree(&buf);
      ...
      HANDLE hFind;
      char duckstation_exec[MAX_PATH];
      char duckstation_dir[256] = "C:\\RetroArch-Win64\\system\\duckstation";
      char searchPath[MAX_PATH];

      snprintf(searchPath, MAX_PATH, "%s\\duckstation*.exe", duckstation_dir);
      ...
      snprintf(duckstation_exec, MAX_PATH, "%s\\%s", duckstation_dir, findFileData.cFileName);
      FindClose(hFind);
   ...
   for (size_t i = 0; i < 4; i++) {
    strncat(duckstation_exec, args[i], strlen(args[i]));
   } 
   ...
   printf("libretro-duckstation-launcher: Failed running duckstation. Place it in the right path and try again\n");
   return false;
}
